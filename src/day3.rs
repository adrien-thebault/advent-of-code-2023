...
fn main() {
    timer!("total");
    let (input, schematic, number_positions, gears_positions): (_, _, Vec<_>, Vec<_>);

    {
        timer!("prepare");
        input = String::from_utf8_lossy(include_bytes!("../inputs/day3.txt"));

        schematic = input.lines().map(|l| l.chars().collect_vec()).collect_vec();
        (number_positions, gears_positions) = input
            .lines()
            .enumerate()
            .flat_map(|(i, l)| {
                l.chars()
                    ...
                    [(i, j - 1), (i, j + n.len())]
                        ...
                        .chain(
                            (j as i32 - 1..=j as i32 + n.len() as i32)
                                .filter(|j| *j >= 0)
                                .flat_map(|j| {
                                    [(i - 1, j as usize), (i + 1, j as usize)].into_iter()
                                }),
                        )
                        .any(|(i, j)| {
                            schematic
